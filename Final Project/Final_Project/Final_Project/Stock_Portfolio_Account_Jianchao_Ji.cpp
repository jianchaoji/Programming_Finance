...
void stockAccount::readFile()
{
	...
	while (ptr != NULL)
	{
		cout << setw(15) << ptr->companyName << setw(15) << ptr->stockPrice << setw(15) << ptr->date << endl;
		ptr = ptr->myNext;
	}
 ...
 void stockAccount::input()
 {
	 ...
	 while (content != NULL)
	 {
		 accountWrite << setw(20) << content->name << setw(10) << content->numberOfShare << setw(10)
			 << content->price << setw(15) << content->totalValue << endl;
 ...
 void stockAccount::buyStock()
{
	...
	if (this->findStock(name) == 1)
	{
		...
		if (this->findStockInAccount(name)==0)
		{
			...
			totlaPrice = (content->numberOfShare)*buyPrice;
			...
			time_t tt = time(NULL);
			tm* t = localtime(&tt);
			accountHistory << left << setw(10) << "BUY" << setw(10) << temp.companyName
				<< setw(10) << content->numberOfShare << setw(10) << temp.stockPrice << setw(10) << totlaPrice
		...
		else if(this->findStockInAccount(name) == 1)
		{
			...
			{
				cout << "You have purchased " << count << "shares of "<< temp.companyName <<" at $" << buyPrice << " each for a total of $" << totlaPrice << endl;
				balance = balance - totlaPrice;
				setBalance();
			}
			accountHistory.open("stockHistory.txt", ios::app);
			...
			tm* t = localtime(&tt);
			accountHistory << left << setw(10) << "BUY" << setw(10) << temp.companyName
				<< setw(10) << count << setw(10) << temp.stockPrice << setw(10) << totlaPrice
				<< " " <<""<< t->tm_hour << ":" << t->tm_min << ":" << t->tm_sec << endl;
			accountHistory.close();
			content->name = temp.companyName;
 ...
 void stockAccount::sellStock()
 {
	 ...
	 else if (this->findStockInAccount(name) == 1)
	 {
		 ...
		 totalPrice = count*sellPrice;
		 cout << "You have sold " << count << "shares of "<< temp.companyName << "at $" << sellPrice << " each for a total of $" << totalPrice << endl;
		 ...
		 time_t tt = time(NULL);
		 tm* t = localtime(&tt);
		 accountHistory << left << setw(10) << "SELL" << setw(10) << temp.companyName
			 << setw(10) << count << setw(10) << temp.stockPrice << setw(10) << totalPrice
			 << " " << "" << t->tm_hour << ":" << t->tm_min << ":" << t->tm_sec << endl;
		 accountHistory.close();
		 content->price = temp.stockPrice;
		 content->totalValue = (content->numberOfShare)*(content->price);
 ...
 void stockAccount::stockAccountHistory()
 {
	 accountHistory.open("Stock_Account_History",ios::app);
